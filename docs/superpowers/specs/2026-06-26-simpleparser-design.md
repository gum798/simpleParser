...
### 공유 타입
```ts
type Format = 'json' | 'html' | 'xml' | 'yaml';
...
3. **`정렬`**: `format(content, fmt)` → `output`이 있으면 에디터 내용 교체(부분 결과 포함), `diagnostics`는 인라인 밑줄 + 상태줄에 표시. 원문이 더 안전하면 보존 정책(아래 8장) 적용.
4. **`트리`**: `buildTree` → `root`가 있으면 트리 패널 토글(부분 트리 포함), `diagnostics` 표시.
...
| YAML | `yaml`(eemeli) `parseDocument` 성공 & 위 셋 아님 | 유효 시 `parseDocument` → `String(doc)`; 무효 시 부분 문서 직렬화 | `parseDocument` → JS 값(복구 포함) | `@codemirror/lang-yaml` |

**감지 순서**: JSON → XML → HTML → YAML → unknown. 수동 드롭다운 선택은 항상 우선.
...
- **유닛(Vitest)**:
  ...
  - `offsetToLineCol`: 경계값
- **스모크(Playwright 1개)**: 빌드된 `dist/index.html` 로드 → 붙여넣기 → 정렬 → 깨진 입력에서 에러 밑줄 확인 → 공유 → 새로고침 복원
...
백엔드·short URL·KV / 포맷 변환기 / CSV·MD·JS·CSS·SQL 등 추가 포맷 / 로그인·서버 히스토리 / Web Worker / 다중 테마

## 12. 프로젝트 구조

```
index.html · vite.config.ts · package.json · tsconfig.json · README.md
src/ main.ts editor.ts detect.ts urlState.ts tree.ts ui.ts types.ts styles.css
     util/ offsetToLineCol.ts
     format/ index.ts json.ts html.ts xml.ts yaml.ts
test/ detect.test.ts format.test.ts urlState.test.ts tree.test.ts e2e.spec.ts
```

## 의존성 요약

- 런타임: `codemirror` + `@codemirror/lang-{json,html,xml,yaml}` + `@codemirror/lint`, `jsonc-parser`, `yaml`(eemeli), `js-beautify`, `xml-formatter`, `lz-string`
